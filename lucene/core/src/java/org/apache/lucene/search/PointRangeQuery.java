    ...
    return new ConstantScoreWeight(this, boost) {
      ...
      public ScorerSupplier scorerSupplier(IndexSearcher.LeafReaderContextPartition partition)
        ...
        if (partLower > queryUpper) {
          return null; // This partition has no values to process
        }

        // Create new bounds for this partition's value range
        ...
        // Check if partition's range intersects with field's actual values
        ...
        return new ConstantScoreScorerSupplier(score(), scoreMode, reader.maxDoc()) {
          ...
          public DocIdSetIterator iterator(long leadCost) throws IOException {
            ...
            // Create visitor that only matches this partition's value range
            IntersectVisitor visitor =
                new IntersectVisitor() {
                  ...
                  @Override
                  public void visit(int docID, byte[] packedValue) {
                    // Check against partition's value range
                  ...
                  @Override
                  public Relation compare(byte[] minPackedValue, byte[] maxPackedValue) {
                    // Check if BKD cell intersects with partition's value range
